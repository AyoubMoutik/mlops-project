...
\midrule
\texttt{NUM\_SAMPLES} & \texttt{100} & Limits training samples for a fast CI execution. \\
\texttt{NUM\_EPOCHS} & \texttt{1} & Controls training epochs. \\
\texttt{BATCH\_SIZE} & \texttt{16} & Controls model training batch size. \\
\texttt{MIN\_F1} & \texttt{0.15} & Minimum weighted F1 score required before deployment can continue. \\
...
The successful Dockerized run produced a weighted F1 of \texttt{0.2014707645577715}, which passed the configured \texttt{MIN\_F1=0.15} threshold. This low threshold is intentional for a fast CI demonstration with \texttt{NUM\_SAMPLES=100} and \texttt{NUM\_EPOCHS=1}.
\end{infobox}
...
The observed model behavior was that the deployed demo model predicted \texttt{other} for all tested scenarios, with low confidence around \texttt{0.07} to \texttt{0.08}. This is not a monitoring failure. It is a useful monitoring finding: the service works, but the model quality is weak because the Jenkins demo run intentionally uses a small sample size and only one training epoch.
...
\begin{itemize}
    ...
    \item The Jenkins training configuration is intentionally small for fast execution.
    \item The final model quality is low because the demo uses \texttt{NUM\_SAMPLES=100} and \texttt{NUM\_EPOCHS=1}.
...
\begin{itemize}
    \item Increase training samples and epochs for stronger model quality.
    \item Add stricter deployment thresholds once model performance improves.
